...
/*   Updated: 2022/12/16 07:47:20 by juyojeon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_b_ulltox(size_t addr, t_para *para, int *p_count, int len_ad)
{
	int	count_zero;
...
int	ft_write_ptr(va_list ap, t_para *para, int *print_count)
{
	...
	if ((para->flag & FLAG_MINUS) == 0)
		if (ft_space_for_ptr(para, print_count, len_addr) == -1)
			return (-1);
	if (ft_b_ulltox(addr, para, print_count, len_addr) == -1)
		return (-1);
